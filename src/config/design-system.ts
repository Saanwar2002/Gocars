/**
 * GoCars Design System Configuration
...
export const getColor = (color: string, shade?: number) => {
  ...
  const colorObj = (designSystem.colors as any)[colorName];
  
  if (!colorObj) return color;
  if (typeof colorObj === 'string') return colorObj;
  
  const targetShade = shade || parseInt(colorShade) || 500;
  return colorObj[targetShade] || colorObj[500] || color;
};
